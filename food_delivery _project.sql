...
-- top 3 outlets by cuisine type without using top or limit.
with cte as(
select  restaurant_id,cuisine,count(order_id) as no_of_orders from orders
group by restaurant_id,cuisine
)

,cte2 as (select *, DENSE_RANK()over(partition by cuisine order by no_of_orders desc) as d_rnk from cte)

select * from cte2 where d_rnk<=3

-- find daily new customer count
with first_order as(select customer_code,cast(min(placed_at) AS DATE) as first_order_date from orders
group by Customer_code)

select first_order_date,count(a.customer_code) as cnt from first_order a
join orders b
on a.customer_code=b.customer_code and a.first_order_date=cast(b.Placed_at As Date)
...
-----
with cte as (select Customer_code,cast(min(placed_at) AS DATE) as first_order_date,cast(max(placed_at) AS DATE) as recent_order_date from orders
group by Customer_code)

select c.*, a.promo_code_name from cte c
join orders a
on c.customer_code=a.Customer_code and c.first_order_date=cast(a.Placed_at As Date)
where recent_order_date <  cast(DATEADD(day,-7,getdate())  As Date) and first_order_date <cast(DATEADD(month,-1,getdate())  As Date)
...
select Customer_code,count(*) as total_orders,count(Promo_code_Name) as promo_orders from orders
group by Customer_code
...
with cte as(select customer_code,cast(min(placed_at) as date)  as first_order_date from orders
group by customer_code
having month(cast(min(placed_at) as date))=1 and year(cast(min(placed_at) as date))=2025)

,cte2 as(select a.*
from orders a
join cte b
on a.customer_code=b.Customer_code and cast(a.Placed_at as date) =b.first_order_date)

select round(count(*)*100.0/(select count(*) from cte2),2) as precentage_organic_cust from cte2
